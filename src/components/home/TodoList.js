...
function itemAction(action, item, setLoading, setTodoListItems) {
  ...
  //const API_URL = `https:///tasks/${item.userId}/${item.id}`
  const API_URL = `https://todo-too-dgs2.web.app/tasks/${item.userId}/${item.id}`;
  //const API_URL = `http://localhost:5000/tasks/${item.userId}/${item.id}`;
  const params =
    action === "done"
      ? {
          method: "PATCH",
    ...
    <List.Item
      actions={[
        <Button
          primary
          key="list-done"
          onClick={() => itemAction("done", item, setLoading, setTodoListItems)}
        >
          done
        </Button>,
        <Button
          ...
          key="list-delete"
          onClick={() =>
            itemAction("delete", item, setLoading, setTodoListItems)
          }
        >
          delete
        </Button>,
